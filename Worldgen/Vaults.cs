...
namespace CSkies.Worldgen
{
    public class Vault : MicroBiome
    {
        public override bool Place(Point origin, StructureMap structures)
        {
            ...
            {
                WorldGen.PlaceObject(origin.X + 13, origin.Y + 14, mod.TileType("HeartAltar2"));
                NetMessage.SendObjectPlacment(-1, origin.X + 13, origin.Y + 14, mod.TileType("HeartAltar2"), 0, 0, -1, -1);
            }
            else if (CWorld.VaultCount == 1)
            {
                WorldGen.PlaceObject(origin.X + 13, origin.Y + 14, mod.TileType("HeartAltar3"));
                NetMessage.SendObjectPlacment(-1, origin.X + 13, origin.Y + 14, mod.TileType("HeartAltar3"), 0, 0, -1, -1);
            }
            else if (CWorld.VaultCount == 2)
            {
                WorldGen.PlaceObject(origin.X + 13, origin.Y + 14, mod.TileType("HeartAltar4"));
                NetMessage.SendObjectPlacment(-1, origin.X + 13, origin.Y + 14, mod.TileType("HeartAltar4"), 0, 0, -1, -1);
            }
    ...
    public class VaultPlanet : MicroBiome
    {
        public override bool Place(Point origin, StructureMap structures)
        {
            ...
            Dictionary<Color, int> colorToWall = new Dictionary<Color, int>
            {
                [new Color(0, 0, 255)] = mod.TileType("AbyssWall"),
                [Color.Black] = -1
            };
            ...
            WorldGen.PlaceObject(origin.X + 17, origin.Y + 18, mod.TileType("HeartAltar1"));
