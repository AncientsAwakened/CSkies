...
namespace CSkies.Tiles.Abyss
{
    class AbyssSandBall : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            {
                int dust = Dust.NewDust(projectile.position, projectile.width, projectile.height, ModContent.DustType<Dusts.VoidDust>(), 0f, 0f, 0);
        ...
        public override void Kill(int timeLeft)
        {
            if (projectile.owner == Main.myPlayer && !projectile.noDropItem)
            {
                ...
                if (!Main.tile[tileX, tileY].active())
                {
                    ...
                    if (!onMinecartTrack && Main.tile[tileX, tileY].active() && Main.tile[tileX, tileY].type == ModContent.TileType<AbyssSand>())
                    {
                        ...
                        {
                            NetMessage.SendData(MessageID.TileChange, -1, -1, null, 1, tileX, tileY, ModContent.TileType<AbyssSand>(), 0, 0, 0);
                        }
                    }
                }
            }
