...
namespace CSkies.NPCs.Minibosses.Observatory
{
    internal class SecurityDroneShock : ModProjectile
    {
        ...
        readonly int frameHeight = 50;

        public override bool PreAI()
        {
            if (head == -1)
            {
                int npcID = BaseAI.GetNPC(projectile.Center, mod.NPCType("SecurityDrone"), 500f, null);
                if (npcID >= 0) head = npcID;
            }
            if (head == -1) return false;
            NPC headNPC = Main.npc[head];
            if (headNPC == null || headNPC.life <= 0 || !headNPC.active || headNPC.type != mod.NPCType("SecurityDrone")) { projectile.Kill(); return false; }
