...
namespace CSkies.NPCs.Bosses.Enigma
{
    public class EnigmaBeam : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            if (enigma.active && enigma.type == mod.NPCType("Enigma"))
            {
                projectile.Center = enigma.Center + Vector2.UnitX.RotatedBy(((Enigma)enigma.modNPC).handRot);
            }
            else
            {
                projectile.Kill();
            ...
            if (projectile.localAI[0] >= (ChangeRate + 30))
            {
                projectile.Kill();
                return;
            }
            projectile.scale = (float)Math.Sin(projectile.localAI[0] * 3.14159274f / (ChangeRate + 30)) * 5f * num801;
            if (projectile.scale > num801)
            {
                projectile.scale = num801;
            }
            float num804 = ((Enigma)enigma.modNPC).handRot;
        ...
        public override bool PreDraw(SpriteBatch spriteBatch, Color lightColor)
        {
            ...
            Texture2D texture2D20 = mod.GetTexture("NPCs/Bosses/Enigma/EnigmaBeam2");
            Texture2D texture2D21 = mod.GetTexture("NPCs/Bosses/Enigma/EnigmaBeam3");
