...
namespace CSkies.NPCs.Bosses.FurySoul
{
    public class FireDragon : ModNPC
    {
        ...
        public override void ReceiveExtraAI(BinaryReader reader)
        {
            ...
            if (Main.netMode == 1)
            {
                Fire[0] = reader.ReadFloat();
                Fire[1] = reader.ReadFloat();
            }
        }
        ...
        public override void AI()
        {
            ...
            if (npc.dontTakeDamage)
            {
                for (int j = 0; j < 2; j++)
                {
                    int dust = Dust.NewDust(new Vector2(npc.position.X, npc.position.Y), npc.width, npc.height, DustID.SolarFlare, 0f, 0f, 100, default, 2f);
            ...
            if (Main.netMode != 1)
            {
                if (npc.ai[0] == 0f)
                {
                    ...
                    for (int l = 0; l < 30; l++)
                    {
                        int type = ModContent.NPCType<FireDragonBody>();
                        ...
                        Main.npc[newNPC].localAI[2] = 1;
                        if ((l - 2) % 4 == 0 && l < 26)
                        {
                            Main.npc[newNPC].localAI[2] = 0;
                        }
                        else if (l == 27)
                        {
                            Main.npc[newNPC].localAI[2] = 2;
                        }
                        else if (l == 28)
                        {
                            Main.npc[newNPC].localAI[2] = 3;
                        }
                        else if (l == 29)
                        {
                            Main.npc[newNPC].localAI[2] = 4;
                        }
            ...
            else
            {
                ...
                if (Fire[0] > 200)
                {
                    BaseAI.ShootPeriodic(npc, player.position, player.width, player.height, ModContent.ProjectileType<FireBreath>(), ref Fire[1], 5, npc.damage / 4, 10, true);
                    if (Fire[0] < 320)
                    {
                        Fire[0] = 0;
                    }
        ...
        public override void NPCLoot()
        {
            ...
            {
                int num469 = Dust.NewDust(npc.Center, npc.width, 1, DustID.SolarFlare, -npc.velocity.X * 0.2f, -npc.velocity.Y * 0.2f, 100, default, 2f);
                Main.dust[num469].noGravity = true;
                Main.dust[num469].velocity *= 2f;

                num469 = Dust.NewDust(npc.Center, npc.width, npc.height, DustID.SolarFlare, -npc.velocity.X * 0.2f, -npc.velocity.Y * 0.2f, 100, default);
                Main.dust[num469].velocity *= 2f;
            }
        }
    ...
    public class FireDragonBody : FireDragon
    {
        public override string Texture => "CSkies/NPCs/Bosses/FurySoul/FireDragonBody";

        public override void SetStaticDefaults()
        {
            DisplayName.SetDefault("Fiery Dragon");
        }
        ...
        public override bool PreAI()
        {
            ...
            if (Main.npc[(int)npc.ai[1]].alpha < 128)
            {
                if (npc.alpha != 0)
                {
                    ...
                    {
                        int num935 = Dust.NewDust(new Vector2(npc.position.X, npc.position.Y), npc.width, npc.height, DustID.SolarFlare, 0f, 0f, 100, default, 2f);
